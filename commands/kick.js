module.exports = {
    run: async (message, args) => {
        ...
        if (message.guild.roles.highest.comparePositionTo(member.roles.highest) < 1 && message.author.id !== message.guild.ownerID) return message.channel.send("Vous ne pouvez pas exclure ce membre")
        if (!member.kickable) return message.channel.send("Le bot ne peut pas exclure ce membre")
        const reason = args.slice(1).join(" ") || "Aucune raison fournie"
        await member.kick(reason)
        message.channel.send(`${member.user.tag} a été éxclu pour ` + reason)
    },
    name: "kick",
    guildOnly: true 
}